...
#define MAX_CHANNEL_VOLUME (MAX_VOLUME / 4)
...
static void Update_Ch1(APU *a, int cycles){
    ...
    frequency = Mem_ReadByte(a->memory, NR13_ADDR);
    frequency |= (Mem_ReadByte(a->memory, NR14_ADDR) & 0x7) << 8;
    a->frame_countdown[0] = (2048 - frequency) * 4;

    wave_duty = Mem_ReadByte(a->memory, NR11_ADDR) >> 6;
    ...
    a->audio_buffer[a->sample_number] = MAX_CHANNEL_VOLUME * square_wave_table[wave_duty][a->sequence[0]];
    a->audio_buffer[a->sample_number + 1] = MAX_CHANNEL_VOLUME * square_wave_table[wave_duty][a->sequence[0]];
}

static void Update_Ch2(APU *a, int cycles){
    int frequency, wave_duty;

    frequency = Mem_ReadByte(a->memory, NR23_ADDR);
    frequency |= (Mem_ReadByte(a->memory, NR24_ADDR) & 0x7) << 8;
    a->frame_countdown[1] = (2048 - frequency) * 4;

    wave_duty = Mem_ReadByte(a->memory, NR21_ADDR) >> 6;
    ...
    a->audio_buffer[a->sample_number] = MAX_CHANNEL_VOLUME * square_wave_table[wave_duty][a->sequence[1]];
    a->audio_buffer[a->sample_number + 1] = MAX_CHANNEL_VOLUME * square_wave_table[wave_duty][a->sequence[1]];
}
